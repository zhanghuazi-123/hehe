...
Hehe 不是传统的一问一答式聊天程序，它会以 `TICK` 驱动的方式持续运行，在有外部消息时优先响应，在空闲时依据记忆、任务和上下文继续思考。项目内置了记忆系统、上下文注入、Web 面板、SSE 事件流，以及用于观察"意识流"的监控页面。

<img src="https://github.com/zhanghuazi-123/hehe/blob/main/images/demo.gif" style="width:100%; max-width:1080px;" />
...
从 [Releases](https://github.com/zhanghuazi-123/hehe/releases) 下载安装包，双击安装。安装完成后：

1. 从开始菜单或桌面图标启动 **Hehe**
2. 首次打开自动进入激活页，粘贴一个 LLM API Key（默认推荐 MiniMax），点激活
3. 激活通过后自动进入主界面 `brain-ui`，开始思考

激活信息会保存在 `%APPDATA%\Hehe\config.json`，记忆和沙盒也都存在同一目录下，升级或重装不会丢。
...
> 打包前要确保 `electron-builder install-app-deps` 能顺利跑完（重建 `better-sqlite3` 给 Electron ABI）。如果被 "resource busy" 挡住，说明有正在运行的 Hehe/Jarvis 占着 `.node` 文件，先把它停掉即可。
...
无需安装任何额外工具，直接在 Brain UI 内扫码即可将个人微信账号接入 Hehe。
...
绑定成功后，微信好友 / 群聊发来的消息会自动进入 Hehe 处理并回复。
...
```bash
python scripts/send.py "你好 Hehe"
...
```bash
cd ./hehe
node --env-file=.env ./hehe/scripts/reset.js
```
...
删掉 `%APPDATA%\Hehe\config.json`（Windows）/ `~/Library/Application Support/Hehe/config.json`（Mac）/ `~/.config/Hehe/config.json`（Linux）。源码模式下这个文件在项目根目录，同名删除即可。下次启动会重新进入激活页。
